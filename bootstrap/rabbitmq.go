...
func (ts *RabbitMqInitializer) Initialize() {
	...
	ctx.RabbitConn, err = amqp.Dial(fmt.Sprintf("amqp://%s:%s@%s:%s/admin_vhost", c.User, c.Password, c.Addr, strconv.Itoa(c.Port)))
	if nil != err {
		ctx.Logger.Error("new mq conn err.", zap.Any("err", err))
		return
	}
	ctx.RabbitChannel, err = ctx.RabbitConn.Channel()
	if nil != err {
		ctx.Logger.Error("new mq conn err.", zap.Any("err", err))
	}
}
...
func (ts *RabbitMqInitializer) Close() {
	_ = ctx.RabbitChannel.Close()
	_ = ctx.RabbitConn.Close()
}
